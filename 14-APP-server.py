    ...
    def _handle_wa(self):
        ...
        try:
            ...
            if action == 'findChats':
                ...
                # Load messages from Supabase (complete history) instead of Evolution API
                remote_jid = body.get('remoteJid', '')
                limit = body.get('limit', 500)  # Load more from database
                ...
                try:
                    # Query Supabase for complete message history
                    import urllib.request
                    ...
                    if not messages:
                        ...
                        if isinstance(response_data, dict) and 'messages' in response_data:
                            ...
                            response_data = messages_obj.get('records', []) if isinstance(messages_obj, dict) else messages_obj
                        ...
                        log_info('WA', f'⚠️  Fallback to Evolution API: {len(messages)} messages')
