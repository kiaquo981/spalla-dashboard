-- ================================================================
-- Migration: prazo_entrega em ds_producoes
-- Data: 2026-03-24
-- Contexto:
--   Adiciona campo de deadline por produção de dossiê, visível
--   no pipeline, lista e painel da página /dossies.
-- ================================================================

ALTER TABLE ds_producoes
  ADD COLUMN IF NOT EXISTS prazo_entrega DATE DEFAULT NULL;

COMMENT ON COLUMN ds_producoes.prazo_entrega IS
  'Data limite para entrega do dossiê ao mentorado. NULL = sem prazo definido.';

-- Refresh view para expor o novo campo
-- DROP necessário: CREATE OR REPLACE não permite reordenação de colunas
